...
export default function AttendanceModal({closemodal=()=>{},visibility,att}) {
  ...
  return (
    <Modal transparent visible={visibility}>
      <View style={{backgroundColor:"rgba(255,255,255,0.96)",flex:1}}>
         <TouchableOpacity onPress={()=>closemodal()} style={{backgroundColor:"red",marginLeft:20,marginTop:20,width:40,height:40,borderRadius:20,backgroundColor:colors.green,display:"flex",justifyContent:"center",alignItems:"center"}}>
        ...
        <View style={{width:"100%",display:"flex",flexDirection:"row",alignItems:"center",justifyContent:"space-between",paddingHorizontal:rp(2),paddingVertical:rp(2)}}>
              ...
              <Text style={{color:att[0]?.status==="absent"?colors.red:colors.green,fontFamily:fonts.Nregular,fontSize:rp(2.2)}}>{att[0]?.status}</Text>
        </View>
  </View>
